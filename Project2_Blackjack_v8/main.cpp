...
void Menu(int& option){    //Menu for the first page
    ...
    cout << ": ";
    cin >> option;
    if (option < 1 || option > 3){  //If input is not within the requirements
...
void display(){
    const int OPTIONS = 3;
    const string menuOpt[OPTIONS][2] = {    //2D arrays for printing out the menu 
...
bool exitGm(){  //Option to exit game
    ...
    if (ext == 'Y' || ext == 'y'){
        ...
        first();    //If input is N, recalling first function
    }
    return true;
}